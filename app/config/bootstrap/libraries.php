...
/**
 *  ========================== CLOUDBOOK LIBRARIES ==========================
 * Here we add libraries specific to the app for the cloudbook
 */



/**
 *  ====== Open source lithium plugins =======
 ...
 *  These can be found on github 
 */
Libraries::add('ipnlistener');
Libraries::add('cloudinary');

/**
 * Some libraries require some specific mappings to the classes that are used. 
 * Since Twilio encapsulates its PHP classes within a master file by using an autoload function,
 * all we have to do is map that main file, and let the Twilio PHP library do the REST (pun intended). 
 */
/**
 * Some libraries require  specific mappings to gain access to the classes that are used. 
 * Since Twilio encapsulates its PHP classes within a master file by using an autoload function,
 * all we have to do is map that main file, and let the Twilio PHP library do the REST (pun intended). 
 * The include path is relative to the libraries directory in the app
 */
Libraries::add('twilio', array(
"includePath" => "twilio/Twilio.php",
"prefix" => false,
...
Libraries::add('mandrill', array(
	"includePath" => "mandrill/Mandrill",
	"prefix" => false,
...
$resourcesPath = LITHIUM_APP_PATH . '/resources';
if ( is_dir($resourcesPath) && !is_writable($resourcesPath) ) {
    $resourcesPath = sys_get_temp_dir() . '/lightspeed/resources';
    if ( !is_dir($resourcesPath) ) {
        mkdir($resourcesPath, 0777, true);
    }

    $extraPaths = array(
        '/g11n', '/tmp', '/tmp/cache', '/tmp/cache/templates', '/tmp/logs', '/tmp/tests'
    );
...
Libraries::add('app', array('default' => true, 'resources' => $resourcesPath));
