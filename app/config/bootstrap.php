...
/**
 * This file contains configurations for handling different authentication types within the framework
 *
 */
...
/*DEBUG: This is needed for setting the user data in the profile controller  */
LibraryManager::controllerFilter('app\controllers\AppController', '_init', function($self, $params, $chain){
	$sessionRead = Session::key();
	//DEBUG: this can be confusing. The name of the model is Users, but the name of the session is User. Don't accidentally change the name of this session to Users to test things during the debugging process or login will not trace the user name
	if(Session::read('User.last_login_time', array('name'=>'login') )){
		$self->setUserData(Session::read('User', array('name'=>'login') ));
	}
	//php has deprecated these kinds of references
