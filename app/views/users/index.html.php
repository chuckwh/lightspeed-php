  ...
  <div class="col-md-12">
	...
	<div id="container" class="container" class="login-page">
        ...
        <div id="login">
			...
			<?=$this->form->create(null, array('url' => '/users/login', 'name' => 'login_form', 'id' => 'login_form'));?>
			<?=$this->security->requestToken(array('name'=>'cm-token')); ?>
			<fieldset class="home">
        ...
        <div id="registration" class="registration-page" <?php /* if ($home === true) { */?> class="headerWrapper" <?php /*  } */?>>
			...
			<?=$this->form->create(null, array('url' => '/users/register',null, 'class' => 'login-page-registration',  'id' => 'register_form'))?>
			<?=$this->security->requestToken(array('name'=>'cm-token')); ?>
				<fieldset class="home">
				<legend class="archivo">
					Sign up with Cloudbook
						...
						<?=$this->form->field(  'username', array('class' =>'form-control form-control-add-on', 'id'=>'registeras', 'type' => 'text', 'label'=>'Please pick a screen name', 'placeholder' => 'Please use only numbers and letters')); ?>
						<?=$this->form->field('email', array('class' => 'form-control form-control-add-on focus-color','id'=>'email', 'type' => 'email', 'label'=>'Your Email (Nobody else will ever see this)')); ?>
						<?=$this->form->field('password', array('class' =>'form-control form-control-add-on', 'type' => 'password', 'label'=>'Password','id' => 'txtPassword',  'template' => '<div id="txtPasswordDiv"{:wrap}>{:label}{:input}{:error}</div>', 'maxlength' => '16')); ?>
						<?=$this->form->field('password_verify', array('class' =>'form-control form-control-add-on', 'type' => 'password', 'label'=>'Please retype your password')); ?>
						<?=$this->form->hidden('timezone', array('id' => 'timezone', 'type' => 'hidden'));?>
						<?=$this->form->field('location', array('class' =>'form-control form-control-add-on focus-color', 'id'=>'location', 'type' => 'text', 'label'=>'Zip Code', 'template' => '<div class="" {:wrap}>{:label}{:input}{:error}</div>', 'maxlength' => '5')); ?>
