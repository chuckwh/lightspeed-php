...
/* 
 TODO Notice that this page is not styled the same as the add.html.php page. Don't fix it here. 
 Modularize this so that edit and add are on same page. How would you do this? 
...
<div class="row-fluid" data-view="edit">
		<div class="col-md-12">
			<?=$this->form->create($photo, array('type' => 'file', 'class' => 'photo-form ml15', 'id' => 'photo_form', 'name' => 'photo_form'));?>
				<fieldset>
				...
				?>
					...
					?>
					<?=$this->form->field('Say Something', array('value'=> $post->status, 'type'=>'textarea', id=>"status", 'class'=>'form-control',  'rows'=> '6', 'template' => '<div {:wrap}>{:label}<div class="help">Add your thoughts</div>{:input}{:error}</div>')); ?>
					<div class="mt10 help">
					...
					<div class="mt10">
						<div class="input-prepend">
				              ...
				              <input type="hidden" id="user_name" name="user_name" value="<?= $userSession['username']?>">
				              <input type="hidden" id="image_upload" name="image_upload" value="">
				              <input type="hidden" id="image_url" name="image_url" value="">
				              <?=$this->security->requestToken(array('name'=>'cm-token')); ?>
						</div>
				    </div>
				    <?=$this->form->field('credit', array('value'=> $post->credit,'id'=>'credit', 'class' => 'form-control', 'template' => '<div class="mt10" {:wrap}>{:label}<div class="help">Photo not yours? PLEASE provide a credit or URL</div>{:input}{:error}</div>')); ?>
					<div id="progress_bar">
					...
					<div class="mt10">
						<?=$this->form->submit('Save', array( 'id'=> 'update_post',
